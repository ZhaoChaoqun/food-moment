...
final class CloudSyncManager {
    ...
    func uploadPendingRecords(modelContext: ModelContext) async {
        do {
            ...
            guard !unsyncedRecords.isEmpty else {
                Self.logger.debug("[CloudSync] No pending records to upload")
                return
            }

            Self.logger.debug("[CloudSync] Uploading \(unsyncedRecords.count, privacy: .public) records...")
                    ...
                    for data in batch {
                        group.addTask {
                            do {
                                ...
                                CloudSyncManager.logger.error("[CloudSync] Failed to upload record \(data.id, privacy: .public): \(String(describing: error), privacy: .public)")
            ...
            Self.logger.debug("[CloudSync] Successfully uploaded \(uploadedCount, privacy: .public) records")

        } catch {
            Self.logger.error("[CloudSync] Upload failed: \(error.localizedDescription, privacy: .public)")
    ...
    func updatePendingCount(modelContext: ModelContext) {
        do {
            ...
            Self.logger.error("[CloudSync] Failed to count pending records: \(String(describing: error), privacy: .public)")
        }
    }
