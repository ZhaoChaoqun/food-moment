...
final class SharedDataManager: @unchecked Sendable {
    ...
    func saveWidgetData(_ data: WidgetData) {
        guard let url = widgetDataURL else {
            Self.logger.error("[SharedData] Failed to get widget data URL")
            return
        }
        ...
        do {
            ...
            Self.logger.debug("[SharedData] Widget data saved successfully")
        } catch {
            Self.logger.error("[SharedData] Failed to save widget data: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    func loadWidgetData() -> WidgetData? {
        ...
        do {
            ...
            Self.logger.error("[SharedData] Failed to load widget data: \(error.localizedDescription, privacy: .public)")
            return nil
        }
    }
