...
final class SaliencyDetectionService {
    ...
    nonisolated func detectSaliencyCenter(in image: UIImage) async -> CGFloat? {
        ...
        return await Task.detached(priority: .userInitiated) {
            ...
            do {
                ...
                SaliencyDetectionService.logger.error("[Saliency] Handler error: \(error.localizedDescription, privacy: .public)")
                return nil
            }
            ...
            guard let result = request.results?.first as? VNSaliencyImageObservation,
                ...
                SaliencyDetectionService.logger.debug("[Saliency] No salient objects detected")
                return nil
            }
            ...
            if let boundingBox = primaryObject?.boundingBox {
                ...
                SaliencyDetectionService.logger.debug("[Saliency] Detected center Y: \(centerY, privacy: .public)")
                return centerY
            }
    ...
    func calculateImageOffset(saliencyCenterY: CGFloat?, screenHeight: CGFloat) -> CGFloat {
        ...
        Self.logger.debug("[Saliency] Calculated offset: \(offset, privacy: .public) for centerY: \(centerY, privacy: .public)")
        return offset
    }
