...
struct ContentView: View {
    ...
    private func autoRegisterDevice() async {
        ...
        Self.logger.info("Device auth with deviceId: \(deviceId.prefix(8), privacy: .public)...")
        ...
        do {
            ...
            appState.isAuthenticated = true
            Self.logger.info("Device auth succeeded")

            // DEBUG 模式下自动 seed 演示数据
            #if DEBUG
            try? await APIClient.shared.requestVoid(.seedDemo)
            Self.logger.debug("Demo seed completed")
            #endif
        } catch {
            Self.logger.error("Device auth failed: \(error, privacy: .public)")
        }
    }
}
